...
\begin{itemize}
\item Christoffel記号. $\Gamma_{jk}^{i} = g^{i\bar{l}} \partial_j g_{k\bar{l}} $. 
K\"ahler条件から$\Gamma_{jk}^{i} = \Gamma_{kj}^{i}$が成り立つ. 
...
\item Riemann curvature tensorを
\[
...
\label{eq-Sze-2.4}
\lvert\nabla f_r(0)\rvert
\underalign{\text{(Cor. \ref{cor-Sze-2.3})}}{ \leq }
...
$dV$は$\mathbb{R}^n$上の通常の体積測度とし, 形式随伴$L^*$を
\[
...
$\Omega$を有界集合とし, $u_k\colon \Omega\to\mathbb{R}$を関数列で$\lVert u_k\rVert_{C^{k,\alpha}}<C$がある定数$C$に対して成り立つとする.
このとき$l+\beta<k+\alpha$を満たす任意の$l,\beta$に対し, $u_k$の部分列で$C^{l,\beta}$において収束するものが存在する.
\end{thm}
...
ただ結局は$f$がdistributionでいけるらしい. かなり苦労するため省略.  
\begin{proof}
...
$L$がHilbert空間の同型であることを示す. 
これは$L$と$L^{-1}$の連続性のみが問題である. まず$L$が連続なのは, 係数$L$の係数が$L^\infty$より
$$
...
-u
\underalign{\text{(Def. \ref{defn-cpx-Laplace})}}{=}
...
\underalign{\text{(Thm.\ref{thm-fredholm})}}{\overset{\sim}{\to}}\ (\ker L^*)^\perp\cap C^{1,\alpha}= C^{1,\alpha}(X)
\]
は同型写像である.
よって
$
...
\begin{enumerate}
\item $K_X$がampleかつ$\{\omega_0\} \in c_1(K_X)$ならば, $\{ \omega\}=\{\omega_0\}$となるK\"ahler--Einstein計量$\Ric(\omega)=-\omega$が存在する
\item $c_1(K_X) =  0$の時, $\{ \omega\}=\{\omega_0\}$となるK\"ahler--Einstein計量$\Ric(\omega)=0$が存在する
\item $-K_X$がampleの時, ある$\varepsilon>0$があって, $\{ \omega\}=\{\omega_0\}$かつ$\Ric(\omega) \geq \varepsilon \omega$なるK\"ahler計量が存在する.
...
&\underalign{\eqref{eq-MA-equation-ricci}}{\Rightarrow} 
 ...
 &\underalign{}{\Rightarrow} 
  \Ric\left(\omega_0+\frac{\sqrt{-1}}{2\pi}\partial\bar\partial\varphi\right)
   ...
   - \frac{\sqrt{-1}}{2\pi}\partial\bar\partial \varphi
   =-\left(\omega_0 + \frac{\sqrt{-1}}{2\pi}\partial\bar\partial \varphi\right)
 \end{align*}
となり言えた.
...
$-K_X$がampleの時はK\"ahler--Einstein計量を持つとは限らないので次の形でしか書けない. 
\begin{thm}[{=Theorem~\ref{thm-MY-curv}}]
$-K_X$がampleかつK\"ahler--Einstein計量を持てば
\[
\left\{
2(n+1)c_2(X)-nc_{1}^{2}(X)
\right\}c_1(X)^{n-2}
...
\cite{Bando, Tian92, SW16, GKP22, DGP24}より次がわかっている. 
%\footnote{もしかすると$X$がsmoothな場合はこの結果よりも前に知られていたかもしれない. なお\cite{Bando, SW16, GKP22, DGP24}はKLT多様体で示している.}

\begin{thm}[{\cite{Bando, Tian92, SW16, GKP22, DGP24}}]
$-K_X$がampleかつ, K-semistableならば
\[
\left\{
2(n+1)c_2(X)-nc_{1}^{2}(X)
\right\}c_1(X)^{n-2}
...
\item \cite{Miy77}が曲面の場合に$3c_2 \geq c_1(X)^2$を示した. この場合は$K_X$がbigでもよい. 
\item \cite{Yau77}でYauがK\"ahler--Einstein計量の存在を示して, $K_X$がampleの場合のMiyaoka-Yau不等式を示した. $c_1(K_X)=0$の場合もYauの定理である. 
...
\item $-K_X$がampleかつ, K-semistableの場合は, smoothな場合は実質的には\cite{Bando}が初めらしいこれは久本さんに教えてもらった. \cite{SW16}も近いことをやっていて引用すべきな気もする. \cite{Tian92}によるcanonical extension sheafを使った別証明もある. 
\item $-K_X$がampleかつ, K-semistable KLT多様体は\cite{GKP22, DGP24}による. Tianのcanonical extension sheafをつかう. 
\end{itemize}
...
$(X, \omega_0)$をコンパクトK\"ahler多様体とする. 
\begin{enumerate}
\item $-K_X$がnefの時, 任意の$\varepsilon>0$について$\{ \omega\}=\{\omega_0\}$かつ$\Ric(\omega) \geq - \varepsilon \omega$となるK\"ahler計量が存在する.
\item $K_X$がnefの時, 任意の$\varepsilon>0$について$\{ \omega\}=\{\omega_0\}$かつ$\Ric(\omega) \leq \varepsilon \omega_0$なるK\"ahler計量が存在する
\end{enumerate}
\end{cor}
\begin{proof}
...
&\underalign{\eqref{eq-MA-equation-ricci}}{\Rightarrow} 
 ...
 &\underalign{}{\Rightarrow} 
  \Ric\left(\omega_0+\frac{\sqrt{-1}}{2\pi}\partial\bar\partial\varphi\right)
   ...
   - \varepsilon\frac{\sqrt{-1}}{2\pi}\partial\bar\partial \varphi
  \geq -\varepsilon\left(\omega_0 + \frac{\sqrt{-1}}{2\pi}\partial\bar\partial \varphi\right)
 \end{align*}
...
\item この$\mathcal{F}$がalgebraic foliation with rationally connected leavesになる. 特に$\mathcal{F}$からfibration $f \colon \widetilde{X} \dashrightarrow Y$でgeneral fiberのZariski closureが有理連結であるものが誘導される. 
...
\item $-K_X$がnefのgeneric nefnessは\cite{Ou23}で完全解決. 
\end{itemize}
...
\item  \( \alpha_* \colon H_1(X, \mathbb{Z}) \to H_1(A, \mathbb{Z}) \)は全射かつ, finite kernelである. これはAlbanese mapの性質から. 特に $H_1(X, \mathbb{Q})  \cong H_1(A, \mathbb{Q}) $である. 
...
具体的にいうと\(E_H=\R^{2l}\)で, $B_H=(S^1)^{2l}$である. 
一方普遍被覆
\(
\C^k\to A=\C^k/H
\)
も主$H$束であり, $\C^k$は可縮である. 
したがってこれもまた$H$の分類空間となり, classifying map
\(
u\colon A\to B_H
\)
...
群のコホモロジーの性質から
\(
H^2(H,\Q)\cong H^2(B_H,\Q)
\)
なので, これを用いて$n$を以下で定める.
%\footnote{ここは\cite[Theorem 3.23]{Voi}に合わせた. \cite[Theorem 3.23]{Voi}ではもっと一般の状況を考えている.}
\[
n\coloneqq u^{*} \colon \underbrace{H^2(H,\Q)}_{\cong H^2(B_H, \mathbb{Q})} \to H^2(A, \mathbb{Q}).
\]
$u$はhomotopy equivalenceなので, $n$は全単射である. 
また, $H\simeq\Z^{2k}$なので,
\[
H^2(H,\Q)
=
...
さて(3)より\( \alpha^* \colon H^2(A, \mathbb{Q}) \to H^2(X, \mathbb{Q}) \)単射なので, 上と合わせて
 \[  
 H_2(X, \mathbb{Q}) \underset{\alpha_*}{\twoheadrightarrow} 
  H_2(A, \mathbb{Q}) \underset{u_{*}}{\twoheadrightarrow} 
 H_2(H, \mathbb{Q}) 
 \]
 は全射の合成で全射になる. 
 \( G = \pi_1(X) \)とすると, 分類空間を用いて以下の可換図式が成り立つ:
\[
\begin{array}{ccc}
...
\end{array}
\]
よって  \(H_2(G, \mathbb{Q}) \to H_2(H, \mathbb{Q}) \)は全射である. 

まとめると
\begin{itemize}
\item  \( \alpha_* \colon H_1(G, \mathbb{Q}) \to H_1(H, \mathbb{Q}) \)同型. これは(2)と群のホモロジーの性質$H_1(G, \mathbb{Q}) \cong G_{ab}$とHurewiczの定理$H_1(X, \Z) \cong \pi_1(X)_{ab}=G_{ab}$から. 
\item   \( \alpha_* \colon H_2(G, \mathbb{Q}) \to H_2(H, \mathbb{Q}) \)全射. 
\end{itemize}
よって Lemma~\ref{lem-Voi-3.24}より, $\pi_1(X),  \pi_1(A)$ともにnilpotentであるから, 
\[G=\pi_1(X) \to \pi_1(A)=H
\]
...
$-K_X$がnefでKLTの場合の$\pi_1(X_\reg)$のalmost Abelianは全くわかっていない. (多分3次元ならわかっているかも). また$c_1(K_X)=0$の場合に帰着できるかも不明らしい. これは有理連結KLT多様体の$X_\reg$がどうなるかが難しいためである. なお$-K_X$がnefで$X$がKLTの場合は, $\pi_1(X_\reg)$の有限次元表現がalmost Abelianな像を持つことは\cite{GGK19}と\cite{MW21}を用いればわかる.(\cite{IMM24}参照)  
