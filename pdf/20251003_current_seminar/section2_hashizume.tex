...
以上よりconvolutionはdistributionであっても関数のconvolutionと同様に扱うことができる. 

\begin{proof}
%示し方としては, test function \(\varphi \in \mathcal{D}\)について$$(u \ast v) \ast (\varphi \ast \psi)$$

[(a)]
\(\varphi, \psi \in \mathcal{D}\)について, 
\begin{equation}
\label{eq-thm-H-3.9}
...
\begin{itemize}
\item \( S_v \) cptならば,  \ref{thm-H-3.3}(C)より\( v \ast \phi \in \mathcal{D} \). よって
\(
u \ast (\psi \ast (v \ast \phi)) \underset{\ref{thm-H-3.3}(d)}{=} (u \ast \psi) \ast (v \ast \phi).
\)
\item\( S_u \) cptならば, 
\(
u \ast (\psi \ast (v \ast \phi)) \underset{\ref{thm-H-3.3}(d)}{=}  (u \ast \psi) \ast (v \ast \phi).\)
...
(v \ast u) \ast (\phi \ast \psi) 
 \underset{(\ref{eq2-thm-H-3.9})}{=} 
 (v \ast u) \ast (\psi \ast \phi)
= (v \ast \phi) \ast (u \ast \psi),
\end{equation}
もいえる. 一方で$u \ast \psi,  v \ast \phi \in C^{\infty}$なので
\begin{equation}
\label{eq4-thm-H-3.9}
 (u \ast v) \ast (\phi \ast \psi) 
  \underset{(\ref{eq2-thm-H-3.9})}{=} 
 (u \ast \psi) \ast (v \ast \phi) 
  \underset{\text{$C^\infty$のcovolution}}{=} 
   (v \ast \phi) \ast (u \ast \psi)
    \underset{(\ref{eq3-thm-H-3.9})}{=} 
    (v \ast u) \ast (\phi \ast \psi) 
\end{equation}
以上より
\begin{equation}
\label{eq5-thm-H-3.9}
((u \ast v) \ast \phi) \ast \psi 
\underset{\ref{thm-H-2.10}}{=}  
(u \ast v) \ast (\phi \ast \psi)
  \underset{(\ref{eq4-thm-H-3.9})}{=}  
(v \ast u) \ast (\phi \ast \psi)
\underset{\ref{thm-H-2.10}}{=}  
((v \ast u) \ast \phi) \ast \psi.
\end{equation}

今 \(\psi\) を\ref{defn-H-2.11}でのapproximate identity of \(\mathbb{R}^n\)を持つ列\(\{ h_j\}\) とすれば, \ref{lem-H-2.12}より
\[
((u \ast v) \ast \phi)(t)
\underset{\ref{lem-H-2.12}}{=}  \lim_{j \to \infty} (((u \ast v) \ast \phi) \ast h_j)(t)
\underset{(\ref{eq5-thm-H-3.9})}{=}  \lim_{j \to \infty} ((v \ast u) \ast \phi) \ast h_j)(t)\underset{\ref{lem-H-2.12}}{=}  ((v \ast u) \ast \phi)(t).
\]
以上より, 任意の$\phi \in \mathcal{D}$について$ (u \ast v) \ast \phi = (v \ast u) \ast \phi$であるので, \ref{rem-H-3.8}より \( u \ast v = v \ast u \)である.

[(b)]
(a)より\( S_v \) cptとして良い. ($u, v$を入れ替えれるため)
 任意の \(  \phi \in \mathcal{D} \)について
 \[
(u \ast v)(\phi) 
\underset{\ref{defn-H-3.7}}{=}  
 (u \ast (v \ast \widetilde{\phi}))(0) 
 \underset{\ref{defn-H-2.7}}{=}   u(\widetilde{v \ast \widetilde{\phi}}).
\]
また
 \[\mathrm{Supp}(v \ast \widetilde{\phi}) 
 \underset{\ref{thm-H-3.3} (c)\text{の証明内}}{\subset} S_v + \mathrm{Supp}\,\widetilde{\phi}
= S_v - \mathrm{Supp}\,\phi,\]
より, 
\(\mathrm{Supp}(\widetilde{v \ast \widetilde{\phi}}) \subset \mathrm{Supp}\,\phi - S_v\)となる. 
以上より, $ \phi \in \mathcal{D} $について
\[
\mathrm{Supp}\,\phi \cap (S_u + S_v) = \varnothing
\Leftrightarrow 
(\mathrm{Supp}\,\phi - S_v) \cap S_u = \varnothing
\Rightarrow 
 (u \ast v)(\phi) = 0 
 \]
 %\ \text{if } \mathrm{Supp}\,\phi - S_v \cap S_u = \varnothing\quad (\Leftrightarrow \mathrm{Supp}\,\phi \cap S_u + S_v = \varnothing).
よってSupportの定義\ref{defn-H-22}から
$S_{u \ast v} \subset S_u + S_v$である.

[(c)] (b)より \( S_u, S_v, S_w \)のどれか二つがcptならば,  \((u \ast v) \ast w\)
や \(u \ast (v \ast w)\) はwell-definedである. 

\(\phi \in \mathcal{D}\)について
\begin{equation}
\label{eq6-thm-H-3.9}
(u \ast (v \ast w)) \ast \phi
\underset{\ref{defn-H-3.4}}{=}  u \ast ((v \ast w) \ast \phi)
\underset{\ref{defn-H-3.4}}{=} u \ast (v \ast (w \ast \phi)).
\end{equation}

\(S_w\)が cptの場合, \(w \ast \phi \in \mathcal{D}\)なので, 
\[
((u \ast v) \ast w) \ast \phi
\underset{(\ref{eq6-thm-H-3.9}), \ref{rem-H-3.8}}{=}  (u \ast v) \ast (w \ast \phi)
\underset{(\ref{eq-thm-H-3.9})}{=} u \ast (v \ast (w \ast \phi)).
\]
よって\ref{rem-H-3.8}より\(u \ast (v \ast w) = (u \ast v) \ast w\)である. 

\(S_w\)が cptでない場合, \(S_u\) cptなので, 
\[
u \ast (v \ast w)
\underset{(a)}{=} u \ast (w\ast v)
\underset{(a)}{=} (w \ast v) \ast u
\underset{\text{上の議論}}{=} w \ast (v \ast u)
\underset{(a)}{=} (v \ast u) \ast w
\underset{(a)}{=} (u \ast v) \ast w.
\]
よりいえた. 

[(d)] \(\phi \in \mathcal{D}\), $x \in \R^n$について, 
\[
(\delta \ast \phi)(x) 
 \underset{\ref{defn-H-2.7}}{=} 
  \delta(\tau_x \widetilde{\phi})
= (\tau_x \widetilde{\phi})(0)
= \widetilde{\phi}(-x)
= \phi(x),
\]
である. よって, \(\delta \ast \phi = \phi\) である. 
以上より
\begin{align*}
 (D^{\alpha}u)(\phi)
& \underset{\ref{defn-H-2.7}}{=}  (D^{\alpha}u \ast \widetilde{\phi})(0)
 \underset{\ref{thm-H-3.3}(b)}{=}   (u \ast (D^{\alpha}\widetilde{\phi}))(0)
 \underset{\delta \ast \phi = \phi}{=} 
  (u \ast (D^{\alpha}(\delta \ast \widetilde{\phi})))(0) \\
&\underset{\ref{thm-H-3.3}(b)}{=} (u \ast ((D^{\alpha}\delta) \ast \widetilde{\phi}))(0)
\underset{\ref{rem-H-3.8}}{=} ((u \ast (D^{\alpha}\delta)) \ast \widetilde{\phi})(0)
\underset{(a)}{=} (((D^{\alpha}\delta) \ast u) \ast \widetilde{\phi})(0) \\
& \underset{\ref{defn-H-3.7}}{=} ((D^{\alpha}\delta) \ast u)(\phi).
\end{align*}
以上より\( D^{\alpha}u = (D^{\alpha}\delta) \ast u\)でありいえた. 

[(e)] \(S_\delta\) cptなので$u, v, \delta$に関して(c)が使える状況にある. よって, 
\[
D^{\alpha}(u \ast v)
\underset{(d)}{=} (D^{\alpha}\delta) \ast (u \ast v)
\underset{(c)}{=} ((D^{\alpha}\delta) \ast u) \ast v
\underset{(d)}{=}  (D^{\alpha}u) \ast v,
\]
\[
D^{\alpha}(u \ast v)
\underset{(a)}{=} D^{\alpha}(v \ast u)
\underset{\text{上の議論}}{=} (D^{\alpha}v) \ast u
\underset{(a)}{=}  u \ast (D^{\alpha}v),
\]
であるのでまとめると, 
\[
D^{\alpha}(u \ast v)
